...
func (a *Driver) getRwPath(id string) (string, error) {
	if val, ok := a.rwMap[id]; ok {
		...
		if _, err := os.Stat(path.Join(a.dockerRoot, "containers", id)); os.IsNotExist(err) {
			...
			data, err := ioutil.ReadFile(path.Join(a.dockerRoot, "containers", id, "rwconfig.json"))
			if err != nil {
